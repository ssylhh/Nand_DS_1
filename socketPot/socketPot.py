...
class TransferSocket:
    ...
    def potReady(self) -> None:
        try:
            ...
            # self.socket.connect(("192.168.0.11", 6000))
            
            # request = f"192.168.0.11:6000:192.168.0.7:4000"
            request = f"device1:5000:192.168.0.7:4000"            
...
class PhaseLockSocket:
    ...
    def potPLsetting(self) -> None:
        try:
            ...
            # self.socketPL.connect((self.gv.POT_IP, self.gv.PL_PORTNUM)) # 서버에 연결 요청 lhh check points..
            self.socketPL.connect(("192.168.0.11", 6000))
            
            # request = f"192.168.0.11:6000:192.168.0.7:5000"
            request = f"device1:5000:192.168.0.7:5000"
            
            self.socketPL.send(request.encode())  
            time.sleep(0.2)
